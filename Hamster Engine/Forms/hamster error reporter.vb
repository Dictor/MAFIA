...
Public Class ErrorReporter
    ...
    Public Version As New HamsterVersion("Error Reporter", 1, 0, 140209, 0)

    Private hexception As Exception

    Delegate Sub ReportERR(exception As Exception, caller As String, subcaller As String) '크로스 스레드를 위한 대리자
    ...
    Public Sub ReportError(exception As Exception, caller As String, Optional subcaller As String = "General")
        If txtinfo.InvokeRequired Then
            Dim resdel As ReportERR = New ReportERR(AddressOf ReportError)
            txtinfo.BeginInvoke(resdel, exception, caller, subcaller)
        Else
            Me.Show()
