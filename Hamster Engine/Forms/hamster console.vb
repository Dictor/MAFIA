...
Public Class Console
    Public Version As New HamsterVersion("Console", 1, 3, 140209, 0)

    '  Public converlist As String '콘솔 버젼 리스트
    Public Shared projruntime As Long '엔진 시작 시간
    Private Shared concommand() As String '콘솔 명령어 파싱 배열
    Public Shared permissionlevel As Byte '0=DEFAULT, 1=ERROR, 2=INFO 3=DEBUG

    Private Shared lasttext As String '콘솔창에 마지막으로 입력한 항목

    Private Shared txtinpushed As Boolean 'getstring 함수를 위한 consoleinput 버튼 눌림 상태
    Private Shared waitforinput As Boolean 'getstring 함수를 위한 consoleinput 의 함수 무시 허용 변수

    Private Shared ConNetworkSoc As New Networkmanager ' 콘솔 전용 네트워크 매니저

    Delegate Sub Conprint(caller As String, level As Byte, message As String, subcaller As String) '크로스 스레드를 위한 대리자
    Delegate Sub ConprintArr(caller As String, level As Byte, message As String(), subcaller As String, front As String, back As String) '크로스 스레드를 위한 대리자
    ...
    Public Sub Printarrange(caller As String, level As Byte, message As String(), Optional subcaller As String = "General", Optional frontDefMsg As String = "", Optional lastDefMsg As String = "")
        SyncLock (conlist)
            Try
                ...
                If conlist.InvokeRequired Then
                    Dim printdel As ConprintArr = New ConprintArr(AddressOf Printarrange)
                    conlist.BeginInvoke(printdel, caller, level, message, subcaller, frontDefMsg, lastDefMsg)
    ...
    Public Shared Sub consoleinput_Click(sender As System.Object, e As System.EventArgs) Handles consoleinput.Click
        Try
            '0=정보 1=에러 2=디버그
...
nhelp:
    ...
    Private Shared Sub txtin_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles txtin.KeyDown
        Try
    ...
    Private Shared Sub txtin_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtin.TextChanged

    End Sub

    Private Shared Sub Reftimer_Tick(sender As System.Object, e As System.EventArgs) Handles Reftimer.Tick
    ...
    Public Shared Function getstring(showhelpmsg As Boolean, helpmsg As String)
        Try
            Dim returntext
    ...
    Public Shared Function getadminwinpermission()
        Try
